(function() {
  ...
  Grid.prototype.update = function(e) {
    if (e.service == "requests") {
      console.log("Event (" + e.state + "): ", e);
    }
    if (e.state == "expired") {
      this.remove(e);
    } else {
    ...
    this.removeExpiredEvent(e);
  };

  Grid.prototype.removeExpiredEvent = function (e) {
    var me = this;
    var client_time = clock.clock.getTime();
    var expire_time = e.time + (e.ttl * 1000);
    var timeout     = expire_time - client_time;
    setTimeout(function() {
      if (e.service = "requests") {
        e.state = "expired";
        setTimeout(function () {
